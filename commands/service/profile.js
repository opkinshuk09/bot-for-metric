...
module.exports = class Profile extends Command {
  ...
  async run(message, args) {
    ...
    let history = await db.get(`reviews_${message.guild.id}_${user.id}`) || [];
    let bio = await db.get(`bio_${message.guild.id}_${user.id}`) || 'N/A';
    let claimed = await db.get(`claimedStats_${user.id}`) || 0;
    let contentHistory = String();

    let available = await db.get(`status_${user.id}`);
    ...
    let paypal = await db.get(`paypal_${user.id}`) || this.client.language.service.no_paypal;
    let hours = await db.get(`availableHours_${user.id}`) || this.client.language.service.no_hours;

    let totalResponse = await db.get(`totalResponse_${message.guild.id}_${user.id}`) || [];
  ...
  async slashRun(interaction, args) {
    ...
    let totalResponse = await db.get(`totalResponse_${interaction.guild.id}_${user.id}`) || [];
    ...
    let history = await db.get(`reviews_${interaction.guild.id}_${user.id}`) || [];
    let bio = await db.get(`bio_${interaction.guild.id}_${user.id}`) || 'N/A';
    let claimed = await db.get(`claimedStats_${user.id}`) || 0;
    let contentHistory = String();
