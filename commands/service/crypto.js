...
module.exports = class Crypto extends Command {
  ...
  async run(message, args) {
    ...
    if(!user || !crypto || !address || !amount || !service) return message.channel.send({ embeds: [this.client.utils.usage(this.client, message, this.client.cmdConfig.crypto.usage)] });
    ...
    for(let i = 0; i < this.client.embeds.service.crypto.fields.length; i++) {
      embed.addFields([{ name: field[i].title.replace("<symbol>", Object.keys(converted)[2])
