...
module.exports = class Message extends Event {
    ...
    if(message.channel.name.includes(this.client.config.channels.channel_name)) {
      ...
      if(ticketData?.openedAt != null && ticketData?.owner && ticketData?.owner != message.author.id && message.author.bot == false) {
        let opened = ticketData?.openedAt;
        let now = new Date();
        let creationDate = (new Date(opened).getTime());
        let started = creationDate || new Date().getTime();
        let timeDiff = now.getTime() - started;
  
        let recentResp = await db.get(`recentResponse_${message.guild.id}_${message.author.id}`) || [];
        recentResp.unshift(timeDiff);
        let totalResp = await db.get(`totalResponse_${message.guild.id}_${message.author.id}`) || [];
        totalResp.unshift(timeDiff);
        await db.set(`recentResponse_${message.guild.id}_${message.author.id}`, recentResp);
        await db.set(`totalResponse_${message.guild.id}_${message.author.id}`, totalResp);
        await db.set(`autoClaim_${message.channel.id}`, `${message.author.id}`);
        ticketData.openedAt = null;
        await db.set(`ticketData_${message.channel.id}`, ticketData);
      }
    }
